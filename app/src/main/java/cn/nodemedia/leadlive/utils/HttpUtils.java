...
public class HttpUtils {
     ...
     */
    public static void login(String uname, String pwds, int type, AbsCallback callback) {
        ...
        OkHttpUtils.post(Constants.HTTP_LOGIN) // 请求方式和请求url
                .tag(Constants.HTTP_LOGIN) // 请求的 tag, 主要用于取消对应的请求
     ...
     * @param page 分页页数
     */
    public static void getLiveList(int type, int page, AbsCallback callback) {
        TreeMap<String, Object> sortMap = new TreeMap<>(new MapKeyComparator());
        sortMap.put("type", type);
        sortMap.put("page", page);
        sortMap.put("pagesize", 20);

        OkHttpUtils.get(Constants.HTTP_LIVE_GET) // 请求方式和请求url
                .tag(Constants.HTTP_LIVE_GET) // 请求的 tag, 主要用于取消对应的请求
                .params(getHttpParams(sortMap))
                .execute(callback);
    }
     ...
     * @param title    标题
     */
    public static void postLive(int userid, String location, String title, AbsCallback callback) {
        TreeMap<String, Object> sortMap = new TreeMap<>(new MapKeyComparator());
        sortMap.put("userid", userid);
        sortMap.put("location", location);
        sortMap.put("title", title);

        OkHttpUtils.post(Constants.HTTP_LIVE_POST) // 请求方式和请求url
                .tag(Constants.HTTP_LIVE_POST) // 请求的 tag, 主要用于取消对应的请求
                .params(getHttpParams(sortMap))
                .execute(callback);
    }
     ...
     * @param userid 登陆用户ID
     */
    public static void delLive(int id, int userid, AbsCallback callback) {
        TreeMap<String, Object> sortMap = new TreeMap<>(new MapKeyComparator());
        sortMap.put("id", id);
        sortMap.put("userid", userid);

        OkHttpUtils.delete(Constants.HTTP_LIVE_DEL) // 请求方式和请求url
                .tag(Constants.HTTP_LIVE_DEL) // 请求的 tag, 主要用于取消对应的请求
                .content("live")
                .params(getHttpParams(sortMap))
                .execute(callback);
    }

    private static HttpParams getHttpParams(TreeMap<String, Object> treeMap) {
        ...
        httpParams.put("json", JSON.toJSONString(treeMap));
        return httpParams;
    }
