 ...
 */
public abstract class DbBase implements cn.nodemedia.library.db.DbManager {

    private final HashMap<Class<?>, TableEntity<?>> tableMap = new HashMap<Class<?>, TableEntity<?>>();

    @Override
    @SuppressWarnings("unchecked")
    public <T> TableEntity<T> getTable(Class<T> entityType) throws cn.nodemedia.library.db.DbException {
        synchronized (tableMap) {
            ...
            if (table == null) {
                try {
                    ...
                    throw new cn.nodemedia.library.db.DbException(ex);
                }
    ...
    @Override
    public void dropTable(Class<?> entityType) throws cn.nodemedia.library.db.DbException {
    ...
    @Override
    public void dropDb() throws cn.nodemedia.library.db.DbException {
        ...
        if (cursor != null) {
            try {
                ...
                throw new cn.nodemedia.library.db.DbException(e);
            } finally {
                cursor.close();
            }
        }
    ...
    @Override
    public void addColumn(Class<?> entityType, String column) throws cn.nodemedia.library.db.DbException {
        TableEntity<?> table = this.getTable(entityType);
    ...
    protected void createTableIfNotExist(TableEntity<?> table) throws cn.nodemedia.library.db.DbException {
        if (!table.tableIsExist()) {
            synchronized (table.getClass()) {
                if (!table.tableIsExist()) {
                    cn.nodemedia.library.db.sqlite.SqlInfo sqlInfo = cn.nodemedia.library.db.sqlite.SqlInfoBuilder.buildCreateTableSqlInfo(table);
